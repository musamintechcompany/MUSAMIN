...
@section('title', $store->name . ' - Online Store')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-4 sm:space-y-0">
                ...
                <div class="hidden sm:block flex-shrink-0 ml-6">
                    <button id="followBtn" onclick="toggleFollow()" class="follow-btn bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium transition-colors text-base" data-user-id="{{ $store->user->id }}">
                ...
                @foreach($store->products->where('is_active', true) as $product)
                    <a href="{{ route('store.product.show', [$store->handle, $product->id]) }}" class="group cursor-pointer block" data-product data-product-name="{{ $product->name }}" data-product-type="{{ $product->type }}" data-product-category="{{ $product->category }}" data-product-description="{{ $product->description }}" data-product-tags="{{ $product->tags ? implode(' ', $product->tags) : '' }}">
                        <div class="relative">
                            ...
                            @if($product->slashed_price && $product->slashed_price > $product->price)
                                @php
                                    $discount = round((($product->slashed_price - $product->price) / $product->slashed_price) * 100);
                                @endphp
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-between pt-2">
                                <div class="flex flex-col">
                                    @if($product->slashed_price && $product->slashed_price > $product->price)
                                        <span class="text-sm text-gray-500 dark:text-gray-400 line-through">${{ number_format($product->slashed_price, 2) }}</span>
                                    @endif
                            ...
                            @if($product->type === 'physical' && $product->stock_quantity)
