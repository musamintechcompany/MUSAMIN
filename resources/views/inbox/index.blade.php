...
<body class="font-sans antialiased">
    <div class="flex min-h-screen bg-gray-100 dark:bg-gray-900">
        ...
        <div class="w-full md:w-1/3 lg:w-1/4 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col {{ isset($conversation) ? 'hidden md:flex' : 'flex' }}" id="sidebar">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($conv->latestMessage)
                                    <p class="text-xs text-gray-600 dark:text-gray-300 truncate">{{ $conv->latestMessage->message }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div id="conversationMenu" class="hidden absolute bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-50 min-w-32">
                <button onclick="blockUserFromMenu()" class="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <div class="flex-1 flex flex-col {{ isset($conversation) ? 'flex' : 'hidden md:flex' }}" id="chatArea">
            ...
            <div class="chat-header bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3">
                <div class="flex items-center justify-between">
                    ...
                    <!-- 3-dots menu moved to header -->
                    <div class="relative">
                        <button onclick="toggleChatMenu()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2">
                            <i class="fas fa-ellipsis-v"></i>
                        </button>
                        <div id="chatMenu" class="hidden absolute right-0 top-10 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-10 min-w-32">
                            @if($hasBlocked)
                                <button onclick="unblockUser('{{ $otherUser->id }}', {{ json_encode($otherUser->name) }})" class="block w-full text-left px-3 py-2 text-sm text-green-600 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <i class="fas fa-check mr-2"></i>Unblock
                                </button>
                            @else
                                <button onclick="blockUser('{{ $otherUser->id }}', {{ json_encode($otherUser->name) }})" class="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <i class="fas fa-ban mr-2"></i>Block
                                </button>
                            @endif
            ...
            <!-- Message Input / Block Status -->
            <div class="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">
                @if($hasBlocked)
                    <div class="text-center py-4">
                        <div class="flex items-center justify-center space-x-2 text-red-500 mb-3">
                            <i class="fas fa-ban"></i>
                            <span class="text-sm">You blocked this user</span>
                        </div>
                        <button onclick="unblockUser('{{ $otherUser->id }}', {{ json_encode($otherUser->name) }})" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm">
                            Unblock to chat
                        </button>
                    </div>
                @elseif($isBlockedBy)
                    <div class="text-center py-4">
                        <div class="flex items-center justify-center space-x-2 text-red-500">
                            <i class="fas fa-ban"></i>
                            <span class="text-sm">You have been blocked by this user. You can't send any message to this user.</span>
                        </div>
                    </div>
                @else
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            conversationItems.forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    // Get user name from this conversation item
                    const userName = this.querySelector('h3').textContent;
                    const userImage = this.querySelector('img');
                    const userAvatar = this.querySelector('.bg-gray-300');
                    ...
                    if (headerImage && userImage) {
                        headerImage.src = userImage.src;
                        headerImage.alt = userName;
                    }
        ...
        function blockUserFromMenu() {
            if (!confirm(`Are you sure you want to block ${currentMenuData.userName}?`)) return;
            ...
            fetch('/inbox/block-user', {
                ...
                body: JSON.stringify({ user_id: currentMenuData.userId })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
        ...
        document.addEventListener('click', function(e) {
            if (!e.target.closest('[onclick="toggleChatMenu()"]') && !e.target.closest('#chatMenu')) {
