        ...
        <div class="w-full max-w-sm space-y-4">
            ...
            <form method="POST" action="{{ route('register') }}" id="registerForm" class="space-y-4">
                ...
                <h2 class="text-2xl font-bold text-center text-gray-800 dark:text-white">
                    Create Your {{ config('app.name') }} Account<br>
                    <span class="text-sm font-normal text-gray-600 dark:text-gray-400">Enter your details below to create your account</span>
                </h2>
                ...
                <div class="relative">
                    ...
                    <div id="strengthMeterContainer" class="hidden">
                        <div class="h-2 mt-2 overflow-hidden bg-gray-200 rounded dark:bg-gray-700">
                            <div id="passwordStrengthBar" class="w-0 h-full transition-all duration-300 bg-red-500"></div>
                        </div>
                        <p id="strengthText" class="mt-1 text-xs text-gray-600 dark:text-gray-400"></p>
                    </div>
                ...
                <div class="relative">
                    ...
                    <span id="confirmError" class="hidden text-sm text-red-500 dark:text-red-400">Passwords do not match</span>
                </div>

                <!-- Admin Warning -->
                @if(isset($termsPrivacyStatus) && $termsPrivacyStatus['admin_warning'])
                    <div class="p-3 bg-yellow-100 border border-yellow-400 rounded-md dark:bg-yellow-900 dark:border-yellow-600">
                        <div class="flex items-center">
                            <i class="fas fa-exclamation-triangle text-yellow-600 dark:text-yellow-400 mr-2"></i>
                            <p class="text-sm text-yellow-800 dark:text-yellow-200">
                                <strong>Admin Notice:</strong> Terms & Privacy is enabled in settings but disabled in config. 
                                Please enable <code>Features::termsAndPrivacyPolicy()</code> in <code>config/jetstream.php</code>
                            </p>
                        </div>
                    </div>
                @endif
                ...
                <!-- Terms -->
                @if(isset($termsPrivacyStatus) ? $termsPrivacyStatus['should_show'] : Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                    <div>
                        <label for="terms" class="flex items-start">
                            <input type="checkbox" name="terms" id="terms" class="mt-1 mr-2 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600" />
                            <div class="text-sm text-gray-600 dark:text-gray-400">
                                {!! __('I agree to the :terms_of_service and :privacy_policy', [
                                    'terms_of_service' => '<a target="_blank" href="'.route('terms.show').'" class="text-sm underline text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300">'.__('Terms of Service').'</a>',
                                    'privacy_policy' => '<a target="_blank" href="'.route('policy.show').'" class="text-sm underline text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300">'.__('Privacy Policy').'</a>',
                                ]) !!}
                            </div>
                        </label>
                        <span id="termsError" class="hidden text-sm text-red-500 dark:text-red-400">You must agree to the terms</span>
                    </div>
                @endif
                ...
                <p class="mt-4 text-sm text-center text-gray-600 dark:text-gray-400">
                    ...
                    <a href="{{ route('login') }}" class="text-primary-600 hover:underline dark:text-primary-400">Login</a>
                </p>
            </form>
        </div>
    </div>
