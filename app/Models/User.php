...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            ...
            if ($emailVerificationRequired) {
                ...
                $model->verification_code_expires_at = now()->addMinutes(5);
    ...
    {
        if ($this->hasVerifiedEmail()) return true;
    ...
    public function generateNewVerificationCode(): string
    {
        ...
        $this->verification_code_expires_at = now()->addMinutes(5);
        $this->save();
     ...
     */
    public function sendAffiliateWelcomeEmail(int $feesPaid = null): void
    {
        Mail::to($this->email)
            ->queue((new AffiliateJoinSuccess($this, $feesPaid))->onQueue('emails'));
    }

    /**
     * Send affiliate renewal email (QUEUED to "emails" queue)
     */
    public function sendAffiliateRenewalEmail(int $feesPaid = null): void
    {
        Mail::to($this->email)
            ->queue((new AffiliateRenewalSuccess($this, $feesPaid))->onQueue('emails'));
    }
    ...
    /**
     * Initiate email change (reuses existing verification system)
     */
    public function initiateEmailChange(string $newEmail): void
    {
        // Store the new email in pending_email field
        $this->update([
    ...
    /**
     * Complete email change verification
     */
    public function completeEmailChange(string $code): bool
    {
    ...
    public function becomeAffiliate(): array
    {
        $settings = \App\Models\Setting::getSettings();
        $requiredCoins = ($settings->affiliate_program_fee ?? 200) * ($settings->usd_to_coins_rate ?? 100);
        ...
        $affiliate = Affiliate::create([
            ...
            'joined_at' => now(),
            'expires_at' => now()->addMinutes(10), // 10 minutes for testing
            'fee_paid' => $requiredCoins,
        ]);
        ...
        $this->sendAffiliateWelcomeEmail($requiredCoins);
        ...
        \App\Events\Admin\UserJoinedAffiliate::dispatch($this);
        
        return ['success' => true, 'message' => 'Successfully joined affiliate program'];
    }
    
    public function renewAffiliate(): array
    {
        $settings = \App\Models\Setting::getSettings();
        $requiredCoins = ($settings->affiliate_program_fee ?? 200) * ($settings->usd_to_coins_rate ?? 100);
        ...
        $renewalHistory[] = [
            ...
            'new_expiry' => now()->addMinutes(10)->toISOString(),
            'fee_paid' => $requiredCoins,
        ];
        ...
        $this->sendAffiliateRenewalEmail($requiredCoins);
        ...
        \App\Events\Admin\UserRenewedAffiliate::dispatch($this);
    ...
    {
        return $this->hasManyThrough(\App\Models\StoreOrder::class, \App\Models\Store::class, 'user_id', 'store_id');
    }
