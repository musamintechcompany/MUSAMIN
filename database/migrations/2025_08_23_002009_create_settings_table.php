...
return new class extends Migration {
    public function up()
    {
        Schema::create('settings', function (Blueprint $table) {
            ...
            $table->boolean('email_verification_required')->nullable()->default(true)
            ...
            $table->boolean('terms_privacy_required')->nullable()->default(true)
                ->comment('Require terms and privacy policy acceptance');

            // Profile Section Controls
            $table->boolean('profile_information_required')->nullable()->default(true)
            ...
            $table->boolean('browser_sessions_required')->nullable()->default(true)
                ->comment('Allow users to view browser sessions');
            $table->boolean('two_factor_auth_required')->nullable()->default(true)
                ->comment('Allow users to setup two factor authentication');
            $table->boolean('account_deletion_required')->nullable()->default(true)
            ...
            $table->decimal('minimum_withdrawal_amount', 10, 2)->nullable()->default(10.00)
                ->comment('Minimum coins required for withdrawal');

            // Affiliate Program Settings
            $table->decimal('affiliate_program_fee', 10, 2)->nullable()->default(200.00)
                ->comment('Fee to join affiliate program in USD');

            // Ideas Settings
            $table->boolean('ideas_enabled')->nullable()->default(true)
                ->comment('Enable/disable ideas functionality');

            // Inbox Settings
            $table->boolean('inbox_enabled')->nullable()->default(true)
                ->comment('Enable/disable inbox functionality');

            // Support System Settings Below
            // Live Chat Support Settings
            $table->boolean('live_chat_support_enabled')->nullable()->default(true)
                ->comment('Enable/disable live chat support functionality');

            // Request Callback Settings
            $table->boolean('request_callback_enabled')->nullable()->default(true)
                ->comment('Enable/disable request callback functionality');

            // FAQ Settings
            $table->boolean('faq_enabled')->nullable()->default(true)
                ->comment('Enable/disable FAQ functionality');

            // Help Center Settings
            $table->boolean('help_center_enabled')->nullable()->default(true)
                ->comment('Enable/disable help center functionality');

            // WhatsApp Settings
            $table->boolean('whatsapp_enabled')->nullable()->default(true)
